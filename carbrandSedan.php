...
<head>
    <meta charset="UTF-8">
    <title>SEDAN Collection - REVS</title>
    <style>
...
<div class="container">
    <h1 class="header-title">SEDAN <span>Collection</span></h1>

    <?php
    // 1. Get unique brands that have cars categorized as 'SEDAN'
    $brand_query = "SELECT DISTINCT brand FROM cars WHERE type = 'Sedan'";
        ...
        <div class="listing-item">
            <div class="brand-header" onclick="toggleBrand('<?php echo $cleanID; ?>')">
                ...
                <div class="brand-info">
                    <h2><?php echo $bName; ?> <span>SEDAN</span></h2>
            ...
            <div id="<?php echo $cleanID; ?>" class="models-dropdown">
                <?php
                // 2. Fetch all cars for this specific brand that are 'SEDAN' type
                $car_query = "SELECT * FROM cars WHERE brand = '$bName' AND type = 'Sedan'";
                $car_result = $conn->query($car_query);
                
                while ($car = $car_result->fetch_assoc()) {
                ?>
                <div class="car-row">
                    <img src="<?php echo $car['image_path']; ?>" class="car-img" onerror="this.src='uploads/default.webp'">
    ...
    <?php 
        ...
        echo "<p style='text-align:center; color:#666;'>No SEDAN cars found in the database yet.</p>";
    }
    ?>
